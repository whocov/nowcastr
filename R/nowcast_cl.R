...
# - check if df is already aggregated by that unit
#   i.e. the dates are evenly spaced by the time_units
#   (i.e. the delays are regularly distributed):
#    c(min_delay, min_delay+1*time_units, min_delay+2*time_units, ..., max_delay ) )
#   or perhaps the input data was daily and needs weekly aggregation first
#   -> then we notify the user and let them do the transformation first
